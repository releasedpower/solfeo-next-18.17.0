...
const STAFF_START_X = 25
const STAFF_FIVE_LINES_HEIGHT = 40
...
function drawStaticsOverlay(state: State) {
  ...
  const curlyBraceSize = STAFF_FIVE_LINES_HEIGHT * 2 + 50
  const curlyBraceY = state.height / 2 + curlyBraceSize / 2
  drawCurlyBrace(state.ctx, STAFF_START_X - 15, curlyBraceY-125, curlyBraceSize)
...
function getTrebleStaffTopY(state: State) {
  const staffHeight = STAFF_FIVE_LINES_HEIGHT
  return state.height / 2 - 150 - staffHeight //-25
}

function getBassStaffTopY(state: State) {
  return state.height / 2 -100
}
