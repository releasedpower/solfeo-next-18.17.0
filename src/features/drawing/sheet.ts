...
export const STAFF_FIVE_LINES_HEIGHT = 40 //80
export const STAFF_SPACE = STAFF_FIVE_LINES_HEIGHT / 4 //4
export const PIXELS_PER_ROW = STAFF_FIVE_LINES_HEIGHT / 8 //8
const STAFF_LINE_WIDTH = 1 //2
export const PLAY_NOTES_WIDTH = 10 //20
...
export function drawLedgerLines(
  ...
  ctx.lineWidth = STAFF_LINE_WIDTH/2
...
export function drawMusicNote(
  ...
  ctx.save()
  ctx.font = `${STAFF_FIVE_LINES_HEIGHT}px ${MUSIC_FONT}`
  drawSymbol(ctx, glyphs.noteheadBlack, x - STAFF_SPACE / 4, y, STAFF_FIVE_LINES_HEIGHT, color)
  ctx.restore()
}
